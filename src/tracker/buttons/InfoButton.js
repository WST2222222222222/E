...
export default function InfoButton(props) {
    ...
    function renderCountdownContent() {
        ...
        return (
            <div>
                Santa begins preparing for liftoff on <b>{pt_start}</b> and will liftoff on <b>{t_start}</b>.<br></br><br></br>
                In the mean time, you can click on the <i title="Settings"><SettingsIcon sx={{ width: "18px", height: "18px", bottom: "0.06rem", position: "relative" }} /></i> button to configure tracker settings. <br></br>
    ...
    function renderTrackingAlwaysContent() {
        return (<div>
            The <i className="santa-icon-small" title="Santa"></i> icon represents Santa's current location.<br></br>
            The <i className="gift-icon-small" title="Gift Icon"></i> icon represents cities that the Santa has visited. Click on one to learn more about that city! <br></br>
        ...
        <>
            ...
            <MDBModal id="infoModal" appendToBody show={infoModal} setShow={setInfoModal} tabIndex='-1'>
                <MDBModalDialog id="infoModalDialog">
                    <MDBModalContent>
                        <MDBModalHeader>
                            <MDBModalTitle>Welcome to track.easterbunny.cc!</MDBModalTitle>
                            <MDBBtn className='btn-close' color='none' onClick={toggleInfoModal} title="Click to close this modal"></MDBBtn>
                        </MDBModalHeader>
                        <MDBModalBody>
                            track.easterbunny.cc will no longer run after Easter 2024. <a href="/news/#pid-141" target="_blank" rel="noreferrer">Read the announcement here.</a>
                            <hr></hr>
                            { props.dataMgr.routeState === 0 ? renderCountdownContent() : renderTrackingAlwaysContent() }
                            <hr></hr>
                            Got any questions? <a href="/faq/" rel="noopener" target="_blank">Check out our FAQs</a>.<br></br>
                            Follow us on Twitter: <a href="https://twitter.com/bunny_tracking" rel="noreferrer" target="_blank">@bunny_tracking</a><br></br>
                            Track the Easter Bunny's journey live on Twitter: <a href="https://twitter.com/bunny_updates" rel="noreferrer" target="_blank">@bunny_updates</a>
                            <hr></hr>
                            {process.env.REACT_APP_VERSION} ({process.env.REACT_APP_COMMIT}) · &copy; 2023 track.easterbunny.cc<br></br>
