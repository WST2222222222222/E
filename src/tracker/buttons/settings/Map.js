...
export default function MapSettings(props) {
        ...
        if (props.settingsMgr.is_chromium_browser()) {
            return (
                <>
                    ...
                    <small>Because you're using a Chrome-based browser, if your page zoom or device scale is above 100%, you may see faint grey lines on the map. Enable this setting to attempt to hide them at the expense of slightly distorted map features (including gifts looking slightly misaligned) & missing zoom buttons.
                    </small>
                    <hr></hr>
                </>
            )
    ...
    const renderDefaultZLText = () => {
        if (dataMgr.routeState === 0) {
            return "Sets the zoom level of the map when the tracker loads, when the map is centered, and when Santa reaches/departs a stop. This setting will apply when Santa's journey begins."
        } else if (dataMgr.routeState === 1) {
            return "Sets the zoom level of the map when the tracker loads, when the map is centered, and when Santa reaches/departs a stop. Because we're in pre-tracking, this setting doesn't apply until tracking begins."
        } else if (dataMgr.routeState === 2) {
            return "Sets the zoom level of the map when the tracker loads, when the map is centered, and when Santa reaches/departs a stop."
        } else if (dataMgr.routeState === 3) {
            return "Sets the zoom level of the map when the tracker loads, when the map is centered, and when Santa reaches/departs a stop. Because tracking is over, this setting will apply for the next tracker run."
        }
    }

    const renderBouncingEffectText = () => {
        if (dataMgr.routeState <= 1) {
            return "Enable a bouncing effect for Santa while en-route to stops during tracking. This setting will apply when tracking begins."
        } else if (dataMgr.routeState === 2) {
            return "Enable a bouncing effect for Santa while en-route to stops. Santa won't bounce when arrived at a stop."
        } else if (dataMgr.routeState === 3) {
            return "Enable a bouncing effect for Santa while en-route to stops during tracking. Because tracking is over, this setting won't apply until the next tracker run."
        }
    }
    ...
    return (
        <>
            ...
            <hr></hr>
            <h5>Santa bouncing effect</h5>
            <MDBRadio name='santaMagic' id='santaMagic-on' value={true} label='On' inline checked={getSetting("santaMagic")} onChange={onRadioChange} />
            <MDBRadio name='santaMagic' id='santaMagic-off' value={false} label='Off' inline checked={!getSetting("santaMagic")} onChange={onRadioChange} /><br></br>
            ...
            <small>Change whether the tracker zooms in when Santa arrives at a stop and zooms out when departing a stop. Turn this off if you're recording a timelapse of the tracker, if the zoom animation lags on your device, or want to save mobile data.</small>
            <hr></hr>
            { renderGrayLines() }
            ...
            <small>The map will be at zoom level <b>{getSetting("zoomOnStopArrival") ? getSetting("defaultZL_actual") + 3 : getSetting("defaultZL_actual")}</b> when Santa is arrived at a stop.</small><br></br><br></br>
